...
async function loadUpiPayments() {
  ...
  try {
    ...
    if (data.platformError) {
      ...
      routingNotice(`<strong>All ${rows.length} are set up.</strong> Each is paid into its own account.`, 'success');
    } else {
      routingNotice(
        `<strong>${rows.length - ready} of ${rows.length} cannot be paid by ` +
        `${provider === 'stripe' ? 'card' : 'bank transfer'} yet.</strong> ` +
        'Until one is set up, a payment to it is refused rather than misrouted.', 'warning');
    }
    ...
    // Unset-up first: this screen exists to show the gap, not to be tidy.
...
function openRoutingAdd() {
  ...
  card.dataset.merchantReference = '';
  document.getElementById('routing-form-property').textContent = 'a new payee';
  document.getElementById('routing-name').value = '';
  document.getElementById('routing-reference').value = '';
  document.getElementById('routing-reference').readOnly = false;
  document.getElementById('routing-account-hint').textContent = '';
...
function openRoutingForm(merchantReference, name, row) {
  ...
  // The platform chose this reference; typing over it here would only break
  // the link between the property and its money.
  const referenceField = document.getElementById('routing-reference');
  referenceField.value = merchantReference;
  ...
  try {
    const res = await adminFetch('/api/admin/merchant-routing/provision', {
      ...
      body: JSON.stringify({
        ...
        merchant_reference: document.getElementById('routing-reference').value.trim(),
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('routing-add-btn')?.addEventListener('click', openRoutingAdd);
